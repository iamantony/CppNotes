...
BOOST_AUTO_TEST_CASE(test_btpostotr_empty_bt)
{
    ...
    DataStructures::BT::Postorder::Solution solution;
    const std::vector<int> expected;
    BOOST_CHECK(expected == solution.PostorderTraversalRecursive(bt));
    BOOST_CHECK(expected == solution.PostorderTraversalIterStack(bt));
    DeleteBT(&bt);
}
