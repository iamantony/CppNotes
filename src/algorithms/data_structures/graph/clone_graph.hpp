#ifndef CLONE_GRAPH_HPP
#define CLONE_GRAPH_HPP
...
namespace Algo::DS::Graph {
    ...
    // TODO: use recursion
    class CloneGraph {
        static Node* deepClone(Node* node, ) {

        }
    public:
        static Node* clone(Node* node) {
            std::unordered_map<int, Node*> knownNodes;
            std::stack<Node*> nodesToClone, clonedNodes;
            nodesToClone.push(node);
            while(!nodesToClone.empty()) {
                auto* n = nodesToClone.top();
                nodesToClone.pop();

                if (knownNodes.count(n->val) > 0) {
                    auto* createdNode = knownNodes[n->val];
                    if (clonedNodes.empty()) {
                        throw std::exception("Empty cloned nodes stack");
                    }

                    auto* prevNode = clonedNodes.top();
                    prevNode->neighbors.push_back(createdNode);
                }
                else {
                    auto* newNode = new Node(n->val);
                    knownNodes.push_back(newNode->val, newNode);
